...
- WS / alps: quiesce=22816ms, bytes=43.8 MB (0.0613% of the ~70 GB pyramid)

- WS / ljubljana: quiesce=19643ms, bytes=42.6 MB (0.0597% of the ~70 GB pyramid)
...
Bytes downloaded until the view is complete, and the request/throughput rates. The responses/s plateau at small sizes is the fixed per-request cost (the server decodes a whole tile per request); the MB/s plateau at large sizes is the decode/transfer ceiling. `dataset_pct` is the share of the served pyramid one view touches.

Files: `E2_bandwidth.png`, `E2_bandwidth.csv`

## Noise band

Every replicate of the default cell across all loaded experiments: 392.5 ± 12.3 FPS (CV 3.1 %). Differences smaller than ~2 CV are not findings.
